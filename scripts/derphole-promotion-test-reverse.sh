...
start_ms=0
duration_ms=0
remote_base="/tmp/derphole-promotion-reverse-$$"
remote_upload="/tmp/derphole-promotion-reverse-bin-$$"
...
preserve_logs() {
  ...
  local prefix="derphole-reverse-${target//[^A-Za-z0-9_.-]/_}-${size_mib}MiB-${stamp}"
...
cleanup() {
  remote "if [[ -f '${remote_base}.pid' ]]; then kill \$(cat '${remote_base}.pid') 2>/dev/null || true; fi; rm -f '${remote_base}.pid' '${remote_base}.payload' '${remote_base}.err' '${remote_upload}'; if [[ '${remote_bin_dir}' != '${requested_remote_bin_dir}' ]]; then rm -f '${remote_bin}'; fi" >/dev/null 2>&1 || true
  rm -rf "${tmp}"
}

trap 'status=$?; if [[ ${status} -ne 0 ]]; then if [[ ${start_ms} -gt 0 && ${duration_ms} -eq 0 ]]; then end_ms="$(now_ms)"; duration_ms="$((end_ms - start_ms))"; fi; dump_failure; preserve_logs; emit_benchmark_footer 2 false "derphole-promotion-test-reverse-exit-${status}"; cleanup; fi; exit ${status}' EXIT
...
listener_log="${tmp}/receive.err"
listener_out="${tmp}/receive.out"
...
remote_sha="$(remote "sha256sum '${remote_base}.payload' | awk '{print \$1}'")"
...
for _ in $(seq 1 200); do
  ...
  if ! remote "kill -0 \$(cat '${remote_base}.pid') 2>/dev/null" >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
...
start_ms="$(now_ms)"
./dist/derphole --verbose receive --hide-progress --output "${listener_out}" "${token}" >/dev/null 2>"${listener_log}"
...
echo "sender_path_trace=$(echo "${sender_trace}" | paste -sd';' - || true)"
