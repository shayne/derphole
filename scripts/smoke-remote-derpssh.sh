...
remote "rm -f '${remote_base}.share.pid' '${remote_base}.share.out' '${remote_base}.share.err'; env DERPSSH_TEST_AUTO_APPROVE=write DERPSSH_TEST_COMMAND='printf ready; read line; printf input:%s \"\$line\"' nohup '${remote_bin}' --verbose share >'${remote_base}.share.out' 2>'${remote_base}.share.err' </dev/null & echo \$! >'${remote_base}.share.pid'"
...
printf 'hello\n' >"${connect_in}"
"${root_dir}/dist/derpssh" --verbose connect --name smoke "${invite}" <"${connect_in}" >"${connect_out}" 2>"${connect_err}" &
connect_pid=$!
