...
remote_tmp="$(remote 'mktemp -d "${TMPDIR:-/tmp}/derphole-smoke.XXXXXXXXXX"')"
remote_base="${remote_tmp}/derphole-smoke"
...
remote "payload='${payload_remote_to_local}'; split_at=\$(( \${#payload} / 2 )); { printf '%s' \"\${payload:0:split_at}\"; sleep '${transfer_pause}'; printf '%s' \"\${payload:split_at}\"; } | '${remote_bin}' --verbose pipe '${local_token}' >'${remote_base}.sender.out' 2>'${remote_base}.sender.err'"
