...
preserve_logs() {
  ...
  local prefix="iperf-forward-${target//[^A-Za-z0-9_.-]/_}-${size_mib}MiB-${stamp}"
...
trap 'status=$?; if [[ ${status} -ne 0 ]]; then if [[ ${start_ms} -gt 0 && ${duration_ms} -eq 0 ]]; then end_ms="$(now_ms)"; duration_ms="$((end_ms - start_ms))"; fi; dump_failure; emit_benchmark_footer 2 false "iperf-benchmark-exit-${status}"; cleanup; fi; exit ${status}' EXIT
...
\"\${iperf_bin}\" -4 -J -c '${server_host}' -p '${iperf_port}' -n '${expected_size}' -P '${iperf_parallel}' -R
" >"${tmp}/client.json"
wait "${server_pid}"
server_pid=""
end_ms="$(now_ms)"
