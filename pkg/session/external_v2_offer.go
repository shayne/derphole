...
func (rt *externalV2OfferRuntime) send(ctx context.Context, accepted externalV2AcceptedClaim) (retErr error) {
	...
	abortErrCh, stopAbortWatch := rt.watchAbort(ctx, accepted.peerDERP, func() {
		...
		endpointMu.Unlock()
		if endpoint != nil {
			_ = closeExternalV2QUICEndpoint(endpoint, metrics, 0, false, 1, "peer aborted transfer")
		}
	})
...
func (rt *externalV2OfferRuntime) sendQUIC(preferenceCtx context.Context, ctx context.Context, accepted externalV2AcceptedClaim, accept externalV2Accept, tr externalV2ListenTransport, policy ParallelPolicy, managerConnections int, rawDirectBudget time.Duration, countedSrc *byteCountingReadCloser, metrics *externalTransferMetrics, progress *externalV2PeerProgressState, pathEmitter *transportPathEmitter, abortErrCh <-chan error, setEndpoint func(externalV2QUICEndpoint)) error {
	...
	if rawPath.raw {
		...
		if externalV2UsesBulkPacketTransfer(accept.TransferMode) {
			decision := newExternalV2BulkDecisionCoordinator(
				ctx, rt.session.derp, accepted.peerDERP, rt.auth, rt.cfg.Emitter,
			)
			defer decision.Close()
			ctx = decision.Context()
			handled, err := rt.tryOfferSendBulkPacketBlock(
				ctx, accepted, accept, rawPath, metrics, progress, pathEmitter, tr.manager, abortErrCh,
	...
	endpoint, streams, err := rt.openOfferQUICStreams(preferenceCtx, ctx, accepted, tr, rawPath, managerConnections, streamCount, abortErrCh, metrics, setEndpoint)
	if err != nil {
		return err
	}
...
func (rt *externalV2OfferRuntime) tryOfferSendBulkPacketBlock(ctx context.Context, accepted externalV2AcceptedClaim, accept externalV2Accept, rawPath externalV2DirectPacketPath, metrics *externalTransferMetrics, progress *externalV2PeerProgressState, pathEmitter *transportPathEmitter, manager *transport.Manager, abortErrCh <-chan error, options externalV2BulkPacketTransferOptions) (bool, error) {
	if !externalV2UsesBulkPacketTransfer(accept.TransferMode) {
		return false, nil
	}
	emitExternalV2Debug(rt.cfg.Emitter, "v2-block-transfer=bulk-packets")
	batchNative := accepted.claim.BlockPacketBatchCapable && accept.BlockPacketBatchCapable
	grouped := batchNative && externalV2GroupedBulk(accepted.claim.BlockPacketGroupCapable, accept.BlockPacketGroupCapable)
	emitExternalV2BulkPacketRecordMode(rt.cfg.Emitter, grouped)
	err := rt.sendBulkPacketBlock(ctx, accepted, rawPath, batchNative, grouped, metrics, progress, pathEmitter, manager, abortErrCh, options)
...
func (rt *externalV2OfferRuntime) openOfferQUICStreams(preferenceCtx, ctx context.Context, accepted externalV2AcceptedClaim, tr externalV2ListenTransport, rawPath externalV2DirectPacketPath, managerConnections, streamCount int, abortErrCh <-chan error, metrics *externalTransferMetrics, setEndpoint func(externalV2QUICEndpoint)) (externalV2QUICEndpoint, []io.WriteCloser, error) {
	if rawPath.raw {
		...
		setEndpoint(client)
		streams, err := runExternalV2QUICStreamOpen(ctx, true, func(openCtx context.Context) ([]io.WriteCloser, error) {
			return client.OpenStreams(openCtx, streamCount)
		})
		if err != nil {
			err = externalV2StreamOpenFailure(externalV2PreferPeerAbort(preferenceCtx, abortErrCh, err))
			_ = closeExternalV2QUICEndpoint(client, metrics, streamCount, true, 1, err.Error())
			return nil, nil, err
		}
	...
	if err != nil {
		...
		_ = closeExternalV2QUICEndpoint(server, metrics, streamCount, false, 1, err.Error())
		return nil, nil, err
	}
	return server, streams, nil
}

func (rt *externalV2OfferRuntime) sendBulkPacketBlock(ctx context.Context, accepted externalV2AcceptedClaim, path externalV2DirectPacketPath, batchNative, grouped bool, metrics *externalTransferMetrics, progress *externalV2PeerProgressState, pathEmitter *transportPathEmitter, manager *transport.Manager, abortErrCh <-chan error, options externalV2BulkPacketTransferOptions) error {
	...
	stats, err := sendExternalV2BulkBlockPackets(ctx, rt.cfg.BlockSource, externalV2BulkPacketPathFromRaw(path), auth, metrics, options)
...
func (rt *externalV2OfferRuntime) sendQUICStreams(ctx context.Context, accepted externalV2AcceptedClaim, accept externalV2Accept, countedSrc *byteCountingReadCloser, streams []io.WriteCloser, endpoint externalV2QUICEndpoint, abortErrCh <-chan error, metrics *externalTransferMetrics, progress *externalV2PeerProgressState, pathEmitter *transportPathEmitter, manager *transport.Manager) error {
	...
	complete, err := rt.receiveComplete(ctx, accepted.peerDERP, abortErrCh)
	if err != nil {
		_ = closeExternalV2QUICEndpoint(endpoint, metrics, len(streams), false, 1, err.Error())
		return err
	}
...
func (rt *externalV2OfferRuntime) sendStreamData(ctx context.Context, accept externalV2Accept, src *byteCountingReadCloser, streams []io.WriteCloser, endpoint externalV2QUICEndpoint, abortErrCh <-chan error, metrics *externalTransferMetrics) error {
	if externalV2AcceptCarriesBlockTransfer(accept) {
		if err := copyExternalV2SendBlockStreams(ctx, rt.cfg.BlockSource, streams, metrics); err != nil {
			return externalV2SendStreamError(endpoint, abortErrCh, metrics, err)
		}
...
func externalV2SendStreamError(endpoint externalV2QUICEndpoint, abortErrCh <-chan error, metrics *externalTransferMetrics, err error) error {
	_ = closeExternalV2QUICEndpoint(endpoint, metrics, 0, false, 1, err.Error())
...
func (rt *externalV2OfferRuntime) finishSendStream(ctx context.Context, endpoint externalV2QUICEndpoint, complete externalV2Complete, metrics *externalTransferMetrics, progress *externalV2PeerProgressState) error {
	...
	return closeExternalV2QUICEndpoint(endpoint, metrics, 0, false, 0, "complete")
}
...
func (rt *externalV2OfferRuntime) sendAccept(ctx context.Context, peerDERP key.NodePublic, candidates []string, policy ParallelPolicy, claim externalV2Claim) (externalV2Accept, error) {
	...
	if claim.BlockCapable {
		...
		policy := externalV2AcceptedBlockTransferPolicy(claim, validExternalV2BlockSource(rt.cfg.BlockSource), accept.BlockPacketBatchCapable, candidates)
		...
		accept.TransferMode = externalV2SelectOptimizedFileTransferMode(policy.Mode, accept.BlockSize, claim.BlockPacketBatchCapable, accept.BlockPacketBatchCapable)
		accept.TransferMode = externalV2SelectFileTransferMode(accept.TransferMode, accept.BlockSize, claim.DirectTCPFileCapable, accept.DirectTCPFileCapable, claim.DirectTCPFile, accept.DirectTCPFile)
		if externalV2UsesDirectTCPFileTransfer(accept.TransferMode) {
			accept.BlockChunkSize = externalV2DirectTCPChunkSize
		}
		emitExternalV2BlockTransferPolicy(rt.cfg.Emitter, policy)
		emitExternalV2Debug(rt.cfg.Emitter, fmt.Sprintf("v2-file-transfer-selection=policy:%s size:%d claim_batch:%t accept_batch:%t claim_tcp:%t accept_tcp:%t claim_listener:%t accept_listener:%t selected:%s", policy.Mode, accept.BlockSize, claim.BlockPacketBatchCapable, accept.BlockPacketBatchCapable, claim.DirectTCPFileCapable, accept.DirectTCPFileCapable, externalV2DirectTCPAdvertisementUsable(claim.DirectTCPFile), externalV2DirectTCPAdvertisementUsable(accept.DirectTCPFile), accept.TransferMode))
	}
...
func (rt *externalV2OfferReceiveRuntime) openAcceptedReceiveSink(ctx context.Context, accept externalV2Accept, metrics *externalTransferMetrics) (externalV2OfferReceiveSink, error) {
	if externalV2AcceptCarriesBlockTransfer(accept) && rt.cfg.BlockReceiver != nil {
		...
		return externalV2OfferReceiveSink{
			useBlock:     true,
			transferMode: accept.TransferMode,
			batchNative:  accept.BlockPacketBatchCapable && externalV2BulkPacketBatchCapability(),
			grouped:      externalV2GroupedBulk(accept.BlockPacketGroupCapable, externalV2BulkPacketGroupCapability()),
			block:        blockSink,
			blockCfg:     blockCfg,
		}, nil
	}
...
func (rt *externalV2OfferReceiveRuntime) receiveAcceptedQUIC(ctx context.Context, tr externalV2ListenTransport, policy ParallelPolicy, managerConnections int, rawDirectBudget time.Duration, sink externalV2OfferReceiveSink, progressSender *externalV2PeerProgressSender, metrics *externalTransferMetrics, pathEmitter *transportPathEmitter) error {
	if sink.useBlock {
		return rt.receiveQUICBlock(ctx, tr, policy, managerConnections, rawDirectBudget, sink.transferMode, sink.batchNative, sink.grouped, sink.block, sink.blockCfg, progressSender, metrics, pathEmitter)
	}
...
func (rt *externalV2OfferReceiveRuntime) receiveQUIC(ctx context.Context, tr externalV2ListenTransport, policy ParallelPolicy, managerConnections int, rawDirectBudget time.Duration, progressSender *externalV2PeerProgressSender, metrics *externalTransferMetrics, pathEmitter *transportPathEmitter) error {
	endpoint, streams, rawPath, err := rt.acceptReceiveStreams(tr, externalV2StreamCount(policy), managerConnections, rawDirectBudget, metrics)
	if err != nil {
		return err
	}
	defer rawPath.Close()
	if rawPath.raw {
		if err := recordExternalV2OpenQUICPayloadLanes(endpoint, metrics, len(streams), rawPath.addrs, time.Now()); err != nil {
			return err
		}
	}
...
func (rt *externalV2OfferReceiveRuntime) acceptReceiveStreams(tr externalV2ListenTransport, streamCount int, managerConnections int, rawDirectBudget time.Duration, metrics *externalTransferMetrics) (externalV2QUICEndpoint, []io.ReadCloser, externalV2DirectPacketPath, error) {
	...
	if rawPath.raw {
		...
		server.SetManagerConnectionCount(managerConnections)
		streams, err := runExternalV2QUICStreamOpen(tr.ctx, true, func(openCtx context.Context) ([]io.ReadCloser, error) {
...
func (rt *externalV2OfferReceiveRuntime) receiveStreamData(ctx context.Context, endpoint externalV2QUICEndpoint, streams []io.ReadCloser, metrics *externalTransferMetrics) (int64, error) {
	...
	if err != nil {
		return 0, rt.receiveStreamError(endpoint, metrics, err)
	}
	return bytesReceived, nil
}

func (rt *externalV2OfferReceiveRuntime) receiveStreamError(endpoint externalV2QUICEndpoint, metrics *externalTransferMetrics, err error) error {
	_ = closeExternalV2QUICEndpoint(endpoint, metrics, 0, false, 1, err.Error())
...
func (rt *externalV2OfferReceiveRuntime) finishReceiveStream(ctx context.Context, endpoint externalV2QUICEndpoint, bytesReceived int64, progressSender *externalV2PeerProgressSender, metrics *externalTransferMetrics) error {
...
func (rt *externalV2OfferReceiveRuntime) sendClaim(ctx context.Context) error {
	...
	batchCapable := rt.cfg.BlockReceiver != nil && externalV2BulkPacketBatchCapability()
	claim := externalV2Claim{
		...
		BlockPacketBatchCapable: batchCapable,
		BlockPacketGroupCapable: batchCapable && externalV2BulkPacketGroupCapability(),
